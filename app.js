...
const CATEGORIES = ['All','Live Music','Sports','Comedy','Movies','Museums','Markets','Art','Food','Local','Free','Weird'];
...
function sourceBadge(e){
  if(e.source==='Ticketmaster')return '<span class="source-mini live">LIVE</span>';
  if(e.source==='Boston.gov')return '<span class="source-mini official">OFFICIAL</span>';
...
function sourcePriority(e){ return e.source==='Boston.gov'?40:e.source==='Ticketmaster'?34:e.source==='Boston permit'?18:5; }
...
function sourceCount(){ return [state.events.ticketmaster?.length,state.bostonGov.items?.length,state.cityPermits.items?.length].filter(n=>n>0).length; }
function rebuildEventLayer(){
  const live=[...(state.events.ticketmaster||[]),...(state.bostonGov.items||[]),...(state.cityPermits.items||[])].filter(Boolean);
  ...
  const names=[]; if(state.bostonGov.items?.length)names.push('Boston.gov'); if(state.events.ticketmaster?.length)names.push('Ticketmaster'); if(state.cityPermits.items?.length)names.push('City permits');
...
function renderDiscover(){ const filtered=filteredEvents(); const liveCount=state.events.items.filter(e=>!e.preview).length; const best=featuredEvents('best',4),free=featuredEvents('free',4),near=featuredEvents('near',4); return `<header class="header"><div><h1 style="font-size:31px">DISCOVER</h1><div class="subhead">The good stuff, not just the obvious stuff.</div></div><button class="icon-btn" data-event-source title="Event sources">⌁</button></header>
  ...
  <div class="filter-row v4-filters"><select data-when>${WHEN_FILTERS.map(v=>`<option ${v===state.when?'selected':''}>${v}</option>`).join('')}</select><select data-price>${PRICE_FILTERS.map(v=>`<option ${v===state.price?'selected':''}>${v}</option>`).join('')}</select><select data-sort>${SORT_OPTIONS.map(v=>`<option ${v===state.sort?'selected':''}>${v}</option>`).join('')}</select></div>
  <div class="source-strip"><span class="source-chip ${liveCount?'live':''}">${state.events.loading||state.cityPermits.loading||state.bostonGov.loading?'Syncing…':state.events.source}</span><span>${sourceCount()} live sources · ${state.events.deduped} duplicates merged</span></div>
...
async function refreshLive(toast=false){ await Promise.all([loadMbta(),loadBikes(),loadHubArrivals(),load311(),loadBostonGovEvents(),loadCityPermits(),state.ticketmasterKey?loadTicketmasterEvents():Promise.resolve()]);rebuildEventLayer();render();if(toast)showToast(state.mbta.error&&state.bikes.error?'Some live feeds unavailable':'City data refreshed'); }
