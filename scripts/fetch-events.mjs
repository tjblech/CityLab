...
function timeZoneOffsetMs(date,timeZone=BOSTON_TZ){const parts=Object.fromEntries(new Intl.DateTimeFormat('en-US',{timeZone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}).formatToParts(date).filter(p=>p.type!=='literal').map(p=>[p.type,p.value]));return Date.UTC(Number(parts.year),Number(parts.month)-1,Number(parts.day),Number(parts.hour),Number(parts.minute),Number(parts.second))-date.getTime();}
function bostonLocalToIso(year,month,day,hour=0,minute=0,second=0){const localAsUtc=Date.UTC(year,month-1,day,hour,minute,second);let instant=localAsUtc;for(let i=0;i<2;i++)instant=localAsUtc-timeZoneOffsetMs(new Date(instant));return new Date(instant).toISOString();}
function normalizeDateValue(value){
  if(value==null||value==='')return null;
  if(value instanceof Date)return Number.isNaN(value.getTime())?null:value.toISOString();
  const raw=String(value).trim();
  if(/^\d{4}-\d{2}-\d{2}$/.test(raw))return raw;
  let m=raw.match(/^(\d{4})-(\d{2})-(\d{2})T(\d{1,2}):(\d{2})(?::(\d{2})(?:\.\d{1,3})?)?$/);
  if(m)return bostonLocalToIso(Number(m[1]),Number(m[2]),Number(m[3]),Number(m[4]),Number(m[5]),Number(m[6]||0));
  m=raw.match(/^(\d{1,2})\/(\d{1,2})\/(20\d{2})(?:\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?$/i);
  if(m){const month=Number(m[1]),day=Number(m[2]),year=Number(m[3]);if(!m[4])return dateKey(year,month,day);let hour=Number(m[4]),minute=Number(m[5]||0);if(m[6]){if(hour===12)hour=0;if(m[6].toLowerCase()==='pm')hour+=12;}return bostonLocalToIso(year,month,day,hour,minute);}
  const parsed=new Date(raw);
  return Number.isNaN(parsed.getTime())?raw:parsed.toISOString();
}
function normalizeSchemaEvent(e,source,baseUrl,i){const loc=firstArray(e.location)||{};const geo=loc.geo||{};const lat=Number(geo.latitude),lon=Number(geo.longitude);return{id:`${source.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-schema-${i}-${String(e.name||'event').toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,42)}`,title:stripHtml(e.name||'Event'),venue:stripHtml(loc.name||addressText(loc)||source),address:stripHtml(addressText(loc)),start:normalizeDateValue(e.startDate),end:normalizeDateValue(e.endDate),price:offerPrice(e.offers)||'Details',info:stripHtml(e.description||'').slice(0,1000),image:imgUrl(e.image),url:absUrl(e.url||baseUrl,baseUrl),lat:Number.isFinite(lat)?lat:null,lon:Number.isFinite(lon)?lon:null,source};}
...
function parseDateLoose(text=''){const cleaned=stripHtml(text).replace(/\bon\b/gi,' ').replace(/\s+/g,' ');const full=MONTHS.join('|');const short='Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec';const patterns=[new RegExp(`(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\\s+(${full})\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:,)?\\s+(20\\d{2})(?:[^\\d]{0,24}(\\d{1,2}:\\d{2})\\s*(am|pm))?`,'i'),new RegExp(`(${full})\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:,)?\\s+(20\\d{2})(?:[^\\d]{0,24}(\\d{1,2}:\\d{2})\\s*(am|pm))?`,'i'),new RegExp(`(${short})\\s+(\\d{1,2})\\s+(20\\d{2})(?:[^\\d]{0,24}(\\d{1,2}:\\d{2})\\s*(am|pm))?`,'i')];const m=patterns.map(p=>cleaned.match(p)).find(Boolean);if(!m)return null;const month=MONTHS.findIndex(name=>name.toLowerCase().startsWith(m[1].slice(0,3).toLowerCase()))+1,day=Number(m[2]),year=Number(m[3]);if(!month)return null;if(!m[4])return dateKey(year,month,day);let [hour,minute]=m[4].split(':').map(Number);if(hour===12)hour=0;if(String(m[5]).toLowerCase()==='pm')hour+=12;return bostonLocalToIso(year,month,day,hour,minute);}
...
function bostonDateKey(value=new Date()){if(typeof value==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(value))return value;const d=value instanceof Date?value:new Date(value);if(Number.isNaN(d.getTime()))return'';const parts=Object.fromEntries(new Intl.DateTimeFormat('en-US',{timeZone:BOSTON_TZ,year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(d).filter(p=>p.type!=='literal').map(p=>[p.type,p.value]));return `${parts.year}-${parts.month}-${parts.day}`;}
function addDaysToKey(key,days){const [year,month,day]=key.split('-').map(Number);const d=new Date(Date.UTC(year,month-1,day+days,12));return dateKey(d.getUTCFullYear(),d.getUTCMonth()+1,d.getUTCDate());}
function withinHorizon(e,days=HORIZON_DAYS,now=new Date()){if(!e.start)return true;const start=bostonDateKey(e.start);if(!start)return true;const end=bostonDateKey(e.end||e.start)||start;const today=bostonDateKey(now);return end>=today&&start<=addDaysToKey(today,days);}
...
function bostonGovRssItems(xml){return rssBlocks(xml).map((b,i)=>{const title=xmlTag(b,'title')||'Boston event';const link=xmlTag(b,'link')||xmlTag(b,'guid')||SOURCES.bostonGov.home;const desc=xmlTag(b,'description')||xmlTag(b,'content:encoded');if(/abutters meeting|public hearing|commission meeting|board meeting/i.test(title))return null;const iso=(desc.match(/20\d\d[-/]\d\d[-/]\d\d(?:T\d\d:\d\d(?::\d\d)?)?/)||[])[0];const start=normalizeDateValue(iso)||parseDateLoose(`${title} ${desc}`);const loc=(desc.match(/(?:Location|Where):\s*([^|•]{3,100})/i)||[])[1]||'';const address=(desc.match(/\d{1,5}\s+[A-Za-z0-9 .'-]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Way|Parkway|Pkwy|Square|Sq)/i)||[])[0]||'';const free=/\bfree\b/i.test(desc);return{id:`bos-${i}-${title.toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,52)}`,title,venue:(loc||address||'City of Boston').trim(),address,start,price:free?'FREE':'Details',info:desc.slice(0,1000),image:'',url:link,lat:null,lon:null,source:'Boston.gov'};}).filter(Boolean);}
...
function permitRecord(rec,i){const nameKey=findField(rec,[/event.*name/i,/name.*event/i,/event.?title/i,/event/i,/description/i]);const dateKey=findField(rec,[/event.*date/i,/date.*event/i,/start.*date/i,/event.?start/i,/date/i]);const locKey=findField(rec,[/event.*location/i,/location/i,/address/i,/street/i,/site/i]);const statusKey=findField(rec,[/status/i,/decision/i,/approval/i]);const title=(nameKey&&rec[nameKey])||'Boston special event';const rawDate=dateKey&&rec[dateKey];const start=normalizeDateValue(rawDate);if(rawDate&&!start)return null;const status=statusKey?String(rec[statusKey]||''):'';if(/denied|cancel/i.test(status))return null;const address=(locKey&&rec[locKey])||'Boston';return{id:`permit-${i}-${String(title).slice(0,32).replace(/\W+/g,'-')}`,title:String(title).trim(),venue:String(address).trim()||'Boston',address:String(address).trim(),start,price:'Details',info:'Public record from the City of Boston Special Event License Applications dataset. Permit data may describe an event application rather than a complete public event listing.',image:'',url:SOURCES.permits.home,lat:null,lon:null,source:'Boston permit'};}
...
export { bostonDateKey, normalizeDateValue, parseDateLoose, withinHorizon };
